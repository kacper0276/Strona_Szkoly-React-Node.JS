  ...
  useEffect(() => {
    fetch(`${url}/aktualnosci/czytajwiecej/${name}/${id}`)
      ...
      .then((response) => {
        setItem(
          <ReadMoreItem
            alt={response.article[0].alt}
            title={response.article[0].title}
            date={response.article[0].date}
            img={response.article[0].img}
            desc={response.article[0].shortdes}
            longDesc={response.article[0].longdes}
            key={response.article[0].id}
          />
        );
