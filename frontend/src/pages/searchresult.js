...
const SearchResult = () =>{
    ...
    useEffect(()=>{
        axios.post(`${url}/szukaj`, {keywords: keywords})
        ...
        .then(response => {
            ...
            if(response.data.searchData != 'Nie znaleziono szukanej frazy'){
                ...
                response.data.searchData.forEach((item)=>{
                    temp.push(<News title={item.title} date={item.date} img={item.img} desc={item.shortdes} key={item.id} link={item.id} linkName={'cos'}/>);
                });
                setNews(temp);
            }else{
